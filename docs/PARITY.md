...
| I2C | 2 controllers, master + slave, 16-word FIFOs | 2 controllers at slot 6's upper half, master (clock-stretch obeyed) AND slave chairs (7-bit address, 16-deep queues, NACK-at-full backpressure, UE on empty reads), open-drain on PORTA[3:0], all state TMR'd | Cycles 31+37; chairs exclusive per controller by design |
...
| boot | internal osc + 30 ms delay + auto SPI-ROM boot, eFuse config | PORTA-31-strapped autonomous x1 QSPI-MRAM boot through the proven loader (CRC + one ruling + watchdog revert); config straps in the MRAM image page stand in for eFuse - no fuse tech in the open PDK, recorded in SCOPE | Cycle 38; ground UART path untouched |
| boot integrity | (qualified product; internal verification undisclosed) | the loader carries THEOREMS: valid-only-from-verify, GOLDEN absorbing, one-ruling-per-reset at flight straps, proven by unbounded induction over free inputs with reachability as the anti-vacuity gate | Cycle 39; the proof suite self-tests its toolchain first |
| package / power pins | LQFP-128, ~40 power+ground of which ~19 VSS (1:4.6 against 88 signal pins) | planned 118 pads: 74 signal, 22 VSS (12 VSSIO interleaved, 8 core, 2 array), 22 VDD - a 1:3.4 ground ratio, TIGHTER than the yardstick on purpose | Cycle 40; derived from measured cluster power and a 56-output SSO case, not copied from the ratio; contingent on slew-limited IO pads |
