...
| zirh_i2c | the I2C pair (slot 6 upper half): a byte-command master that OBEYS clock stretch, and since Cycle 37 a slave chair at the same pins - seven-bit address, 16-deep queues both ways, a full queue NACKs on the wire (the protocol's own backpressure, nothing lost), empty reads serve all-ones with UE sticky, repeated START honored, master parked while the slave sits | test_i2c, test_top_i2c |
...
| autonomous boot | PORTA 31 strapped high: the die streams its own image from external MRAM over a four-pin x1 QSPI lease (PORTA 30-27) through the SAME loader contract - not one UART byte; pin low, the ground flow is bit-identical | test_top_qspi |
