...
| formal equivalence | every hop is a theorem: RTL-to-stitched proven at pilot scale, the stitch at 2138 triples (2639/2639 points), and place-and-route itself - the confirmed layout's netlist vs what entered the flow, 6915/6915 points with the clock tree and repair buffers collapsing inside the proof; cell functions generated from the PDK liberty, vacuous passes refused by gate | scripts/formal_equiv.sh, make equiv |
...
| zirh_i2c | two I2C masters (slot 6 upper half): byte-command engine, open-drain PORTA alternate functions - the wire is the AND of every pull - a stretching slave is obeyed not fought, every register TMR'd; proven against a bench slave at block and through-the-die scale | test_i2c, test_top_i2c |
| zirh_spi | three SPI masters (slot 7 upper half): all four CPOL/CPHA modes, full duplex, software-owned chip select - no auto-CS guessing protocols wrong - MOSI advancing on the non-sampling edge, every register TMR'd; proven against a mode-aware bench slave at block and through-the-die scale | test_spi, test_top_spi |
| zirh_uart1 | the payload's serial port (slot 7 window 3): programmable 8N1, exact-div bit periods - a +1 convention drifts a clock per bit and a UART's peer keeps absolute time - RXD-read clears valid, glitches shorter than half a bit are not starts, all state TMR'd; proven echoing bench bytes across two UARTs at two rates | test_uart1, test_top_uart1 |
